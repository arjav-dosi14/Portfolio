document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // STANDALONE PREMIUM NAVIGATION INDICATOR
    // ==========================================
    (function initNavigationIndicator() {
        const validSectionIds = ['about', 'skills', 'projects', 'education', 'contact'];
        const sections = validSectionIds.map(id => document.getElementById(id)).filter(Boolean);
        
        const navLinks = Array.from(document.querySelectorAll('#desktop-nav .nav-link')).filter(link => {
            const href = link.getAttribute('href') || link.href || '';
            if (!href.includes('#')) return false;
            const id = href.split('#')[1];
            return validSectionIds.includes(id);
        });
        
        const indicator = document.getElementById('nav-indicator');
        if (!indicator || navLinks.length === 0) {
            console.warn('Nav indicator failed to init: missing indicator or valid links');
            return;
        }

        // Apply smooth transition CSS dynamically (300ms)
        indicator.style.transition = 'transform 300ms cubic-bezier(0.4, 0, 0.2, 1), width 300ms cubic-bezier(0.4, 0, 0.2, 1)';
        indicator.style.opacity = '1';

        let activeLinkId = validSectionIds[0];
        ...
        const observer = new IntersectionObserver((entries) => {
            ...
            if (intersecting.length > 0) {
                // If multiple sections intersect, pick the top-most one
                intersecting.sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top);
                activeLinkId = intersecting[0].target.id;
                moveIndicatorTo(getLinkById(activeLinkId));
            } else if (window.scrollY < 100) {
                // Keep visible and default to first link when at the top of the page
                activeLinkId = validSectionIds[0];
                moveIndicatorTo(getLinkById(activeLinkId));
            }
        }, {
            threshold: 0.2,
        ...
        // 2. Click Handling (immediate update)
        ...
        // 3. Window Resize Recalculation
        window.addEventListener('resize', () => {
            const currentActiveLink = getLinkById(activeLinkId);
            if (currentActiveLink) {
                // Disable transition for an instant snap on resize
        ...
        // 4. Page Refresh / Initial Load Setup
        function initActiveState() {
            let currentId = validSectionIds[0];
            // Determine the active section on load based on scroll position
            ...
            activeLinkId = currentId;
            // Timeout ensures fonts and layout are fully computed
            setTimeout(() => {
                moveIndicatorTo(getLinkById(activeLinkId));
            }, 50);
        }
        ...
        if (document.readyState === 'complete') {
            ...
            window.addEventListener('load', initActiveState);
        }
    })();
    // ==========================================
    ...
    window.copyToClipboard = function(text, element) {
        let textToCopy = text;
        if (element) {
            const displayElement = element.querySelector('.flex-1 p:last-child');
            if (displayElement) {
                textToCopy = displayElement.textContent.trim();
            }
        }
